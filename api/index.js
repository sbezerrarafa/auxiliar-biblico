...
// Função serverless padrão da Vercel
export default async function handler(req, res) {
  // CORS básico (funciona tanto local quanto produção)
  ...
  console.log(`Caminho da Requisição: ${req.url} | Método: ${req.method}`);
  console.log('Corpo bruto recebido:', req.body);
  // ---------------------------------------------

  // Em algumas plataformas o body já vem como objeto, em outras como string
  let body = req.body;
  if (!body || Object.keys(body).length === 0) {
    try {
      body = JSON.parse(req.body);
    } catch (e) {
      // se não der parse, deixa como está
    }
  }

  const { pergunta, tipo } = body || {};
...
- **Aplicação e Conclusão:** Dê sugestões aprofundadas sobre a aplicação e faça um breve resumo conclusivo.
    ...
    systemInstruction =
      'Você é um Auxiliar de Estudos Bíblicos profissional. Responda com base na Bíblia, fornecendo referências.';
  }
  ...
  try {
    ...
    console.log(
      `Chamando Gemini 2.5 Flash com tipo: ${tipo}. Pergunta: "${pergunta.substring(
        0,
        60
      )}..."`
    );
    ...
    // Mantive a ideia de usar response.text, como você já fazia local
    const answer = response.text ? response.text : String(response);
    ...
    console.error(
      'ERRO GRAVE [BACKEND]: Falha na chamada da API Gemini.',
      error.message || error
    );
    // -------------------------------------
    return res
      .status(500)
      .json({ error: 'Ocorreu um erro na comunicação com a IA.' });
  }
}
